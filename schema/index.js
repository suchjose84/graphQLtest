...
const UserType: GraphQLObjectType<UserDocument> = new GraphQLObjectType({
  name: 'User',
  fields: () => ({
    username: {
      type: GraphQLString
    ...
    inventory: {
      type: new GraphQLList(InventoryType),
      resolve(parent: UserDocument, args) {
        return Inventory.find({ username: parent.username });
      }
    }
  })
  ...
  fields: {
    ...
    user: {
      ...
      args: {
        ...
        username: {
          type: GraphQLString}, // Add this argument for querying by username
      },
      resolve(parent, args) {
        if (args.id) {
          // Querying by ID
